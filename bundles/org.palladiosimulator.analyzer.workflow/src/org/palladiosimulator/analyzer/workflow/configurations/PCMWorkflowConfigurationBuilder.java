...
/**
 * Builder class which is able to fill the PCM specific parts of a workflow configuration object
 * based on the PCM filename configuration tab entries.
 *
...
public class PCMWorkflowConfigurationBuilder extends AbstractWorkflowConfigurationBuilder {
     ...
     * 
     * @param configuration
     *            The configuration as created by Eclipse based on the data entered by the user in
     *            the Eclipse Run Dialog
     * @param mode
     *            Either "debug" or "run", passed by Eclipse depending on the used dialog
     ...
     * 
     * @see
     * de.uka.ipd.sdq.workflow.launchconfig.AbstractWorkflowConfigurationBuilder#fillConfiguration(
    ...
    public void fillConfiguration(AbstractWorkflowBasedRunConfiguration configuration) throws CoreException {
        ...
        config.setDeleteTemporaryDataAfterAnalysis(
                getBooleanAttribute(ConstantsContainer.DELETE_TEMPORARY_DATA_AFTER_ANALYSIS));
        // accuracy analysis
     ...
     * 
     * @param config
     *            The configuration object to fill with the PCM model filenames
     * @throws CoreException
     */
